...
//    $branch = $_SESSION['branch']; // Make sure you have the branch value in your session
   ...
   $_SESSION['new'] = $newUserId;
    if (isset($_POST['quotation_number'], $_POST['bill_on'], $_POST['jc_date'], $_POST['quotation_date'], $_POST['peyment'], $_POST['client'],$_POST['branch'], $_POST['instruct'], $_POST['proposed'], $_POST['location'], $_POST['completion'],$_POST['now_remark'],$_POST['jc_billable'], $_POST['selected_items'])) {
        ...
        $pre_varification = $_POST['pre_varification'];
        $pname=$_SESSION['user'];
        $csr=$_SESSION['user'];
        // Insert a new record with the incremented id
        $insert_sql = "INSERT INTO jobcard_main (jc_number,quotation_number,bill_on,jc_date,quotation_date,peyment_terms,client,branch_name,instructed_by,proposed_rate,s_location,completion_before,now_remark,jc_billable,involvements,pre_varification,user,csr) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
        $stmt1 = $conn->prepare($insert_sql);
        $stmt1->bind_param("sssssssssissssssss", $newUserId, $quotation_number, $bill, $jc_date, $quotation_date, $peyment, $client,$branch, $instruct, $proposed, $location, $completion,$now_remark,$jc_billable,$selectedItemsString,$pre_varification,$pname,$csr);
        if ($stmt1->execute()) {
            echo "";
        }else{
...
if(isset($_POST['pre_details'])) {
        ...
        $sql = "INSERT INTO jobcard_items_pv (jc_number,pre_details) VALUES (?,?)";
        $stmt2 = $conn->prepare($sql);
        $stmt2->bind_param("ss", $newUserId,$pre_details);
        if ($stmt2->execute()) {
            echo "<script>alert('JC Pre-Verification Created Successfully'); window.location = 'marketing-jc.php';</script>";
        }else{
...
<div class="content-wrapper">
  ...
  <div class="content-header">
    <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-6">
          <h1 class="m-0">NEW PRE-VERIFICATION</h1>
        </div>
      </div><!-- /.row -->
    </div>
    <!-- /.container-fluid -->
...
<div class="form-group" id="branchContainer" style="display: none;">
    <!-- Branch options will be dynamically added here -->
                  ...
                  <div>
                    <input type="hidden"name="pre_varification"value="pre_varification">
                  </div>
                  
                  <div class="col-md-12">
                    <div style="margin-top:25px;" class="card card-info card-outline">
                      <div class="card-header">
                        <h3 class="card-title">Pre Varifications Details</h3>
                      </div>
                      <div class="card-body">
                        <table class="table table-bordered table-striped" id="dataTable">
                          ...
                          <tr>
                            <th style="width:37%; text-align: center;">Pre Varification Details<span class="text-danger">*</span></th>
                          </tr>
                          </thead>
                           <tbody>
                            <tr>
                            <td style="vertical-align:middle;"><textarea class="form-control" style="width:100%;" name="pre_details" id="pre_details"cols="10"rows="5" required></textarea></td>
